...
%-----------------------------------------------------------------------
% pdco.m: Primal-Dual Barrier Method for Convex Objectives (02 Jun 2011)
%-----------------------------------------------------------------------
...
% 30 Mar 2009: If Method<=3, the Hessian must be diagonal as for the
...
%-----------------------------------------------------------------------
  ...
  Method    = options.Method;     % 1=Cholesky  2=QR  3=LSQR  21=SQD
  ...
  if PriLev > 0
    ...
    fprintf('\n\nMethod   = %8g     (1=chol  2=QR  3=LSQR  21=SQD)\n', Method)
  ...
  case 2
    solver  = '    QR';  head3 = '       QR';
  case 3
    solver  = '  LSQR';  head3 = '  atol   LSQR Inexact';
  case 21
    solver  = '   SQD';  head3 = '      SQD';
  otherwise
    error('Method must be 1, 2, 3 or 21')
  end
  ...
  %%% If Method==21 (SQD), we need to make it the latter.
  ...
  if explicitF
    ...
    if diagHess
       ...
       if mH~=n && nH~=n
          error('For Method==21, pdObj must return a square (sparse) Hessian')
       end
    end
  end
...
% mufirst = mu0*(x0min * z0min);
    ...
    if Method<=3  %%% diagHess
      ...
      elseif Method==2
        ...
        if explicitA            % A is a sparse matrix.
                                % Construct diagonal preconditioner for LSQR
          precon = true;
          AD     = pdMat*sparse( 1:n, 1:n, D, n, n );
          AD     = AD.^2;
          ...
          matvechandle = @(mode, m, n, x) ...
              pdxxxlsqrmat(mode, m, n, x, pdMat, Method, precon, ...
                           pdDDD1, d2, pdDDD3);
        ...
        stophandle = @(istop, atol, arnorm, itn) ...
            pdxxxstop(istop, atol, arnorm, itn, atolmin, fmerit); 
        % Note: r3norm or fmerit must be the 2-norm. 
        ...
        % get rid of the extra output variables from lsqr
        clear dr1norm dr2norm danorm dacond darnorm dxnorm dvar dcov;
 ...
 %  stepx  = min( [stepx1,   stepx2,   stepxL,   stepxU] );
    ...
    if PriLev > 0
      ...
      elseif Method==21
        if PDitns==1, fprintf(' %8g %8g', nnz(L),nnz(U)); end
      else
        %relax
      end
...
function y = pdxxxlsqrmat( mode, mlsqr, nlsqr, x, pdMat, Method, ...
...
%
% pdDDD[13] provide various diagonal matrices for each value of Method.
...
%-----------------------------------------------------------------------
  ...
  if isa(pdMat, 'function_handle')
    ...
    if mode==1,  y = pdMat*x;  else  y = (x'*pdMat)'; end
  end
