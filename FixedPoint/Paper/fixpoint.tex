...
\begin{subequations}
    \label{opt-cond}
	\begin{align}
      YDv^\star(\mu)  + YA^T\eta^+ \nu^\star(\mu)  &= YA^Tr + YD\eta^-\rho  \label{opt-primal-feas}\\
      DY^T y^\star(\mu) &= D\log(v^\star(\mu))                        \label{opt-dual-feas}\\
      (YA^T\eta^+)^Ty^\star(\mu)   &= \log(\nu^\star(\mu))            \label{opt-dual-feas-two}\\
...
\begin{proof} 
    ...
    satisfy $(e^TYD, e^TYA^T\eta^+\nu)^T(v,\nu) = \gamma$, and from the assumptions
    on $Y$ and $K$ it follows that 
    $(e^TYD,e^TYA^T\eta^+)$ is a vector of positive entries. It is also formed
	...
	we have
	\begin{align}
	e^T YD v^\star + e^T YA^T \eta^+ \nu^\star &= e^T YA^T r + e^T YD \eta^- \rho \notag 
	\\ & = e^T YD r + e^T YA^T \eta^+\rho=\gamma, \notag 
	\end{align} 
	and thus $(v^\star,\nu^\star)\in \Omega.$
...
%Note that the value of $YDv^\star+YA^T\eta^+\nu^\star$ is the rate of
%consumption of each chemical species and $YA^Tv^\star+YD\eta^-\nu^\star$ is the
%rate of production of each chemical species. At the fixed point, the equality
%$YDv^\star + YA^T\eta^+\nu^\star= YA^Tv^\star+YD\eta^-\nu^\star$ defines a
...
\begin{proof}
	...
	First, assume that $\hat{\nu}>0$ and observe that $p := (D^{-1}A^T\hat v + \eta^-\hat \nu, 0)$ is 
...
\begin{algorithmic}[1]
  ...
  \WHILE{$\|YK v^\star - (Y K \eta^+ + YD \eta^-) \nu^\star \|_\infty>\tau$}
