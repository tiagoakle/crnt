...
\newcommand{\ab}{a}   % b = YKa,  where a = \ab
...
\author{Santiago Akle%
   ...
   \and Onkar A. Dalal%\footnotemark[2] 
   \and Ronan~Fleming%
...
\begin{abstract}
    ...
    for which a positive fixed point exists. We use existence
    of the fixed point to establish existence of a positive steady state.
...
We model an ensemble of chemical reactions where the rates of reaction are
...
Before we define the model we must state a couple of inconsequential technical
assumptions.  First, we assume that each row of $Y$ is non-negative and has
at least one nonzero entry (which implies that the stoichiometric coefficients
are positive and each species participates in at least one complex).  Second,
each column and row of $A$ must have at least one nonzero entry (which is
equivalent to having a network where each complex is produced by at least one
reaction and consumed by at least one reaction).

We use the existence and uniqueness of the solution of the convex problem 
to define a mapping as follows: Let $b=YK\ab$ for some $\ab \in \R^n$ and observe
that for arbitrary $s \in \R^n$ we can write $b = YA^T(\ab +s) - YD(\ab +
D^{-1}A^Ts)$.  In particular, we choose $s$ positive and large enough so
that $\ab^+ := \ab+s$ and $\ab^- := \ab + D^{-1}A^Ts$ are both positive.
Also, from Definition \ref{consis}, $e^Tb = 0$ and thus
\begin{equation}
  e^TYD\ab^- = e^TYA^T\ab^+.
  \label{massbalanceb}
\end{equation}
Define $\mu:=(r,\rho) \in \R^{m}\times\R$ to be a vector parameter.  Observe that if
the parametric convex optimization problem
\begin{equation}
  \tag{P0}
	\begin{array}{lll}
  \underset{(v,\nu)\in\R^{n}\times\R}{\minimize} & v^T D(\log(v)-\1) + \nu(\log \nu -1) 
\\ \st &  YD v + YA^T\ab^+ \nu = YA^Tr + YD\ab^-\rho
\\	    &\quad (v,\nu) \ge 0                     
	\end{array}
	\label{convex-fix}
\end{equation}
has a positive solution $(v^\star(\mu),\nu^\star(\mu))$, then the first-order optimality
conditions \cite{yinyu}
\begin{subequations}
    \label{opt-cond}
	\begin{align}
      YDv^\star(\mu)  + YA^T\ab^+ \nu^\star(\mu)  &= YA^Tr + YD\ab^-\rho  \label{opt-primal-feas}\\
      DY^T y^\star(\mu) &= D\log(v^\star(\mu))                        \label{opt-dual-feas}\\
      (YA^T\ab^+)^Ty^\star(\mu)   &= \log(\nu^\star(\mu))            \label{opt-dual-feas-two}\\
    (v^\star(\mu),\nu^\star(\mu)) &\ge 0                              \label{opt-primal-bounds}
	\end{align}
\end{subequations}
are well defined, where $y^\star(\mu)$ is the vector of dual variables
...
\eqref{convex-fix} is strictly convex, so for any feasible $\mu:=(r,\rho)$ there is 
a unique minimizer. That is, the mapping 
\begin{equation}
  \mu=(r,\rho) \rightarrow (v^\star(\mu),\nu^\star(\mu))
  \label{mapping}
\end{equation} 
is well defined.

If $\hat \mu = (r, \rho)$ is a fixed point of \eqref{mapping}, then
\eqref{opt-primal-feas} implies
\[
	 YDv^\star(\hat \mu)+YA^T\ab^+\nu^\star(\hat \mu) = YA^Tv^\star(\hat \mu) +
	 	YD\ab^-\nu^\star(\hat \mu)
\]
or equivalently
\[
    Y K v^\star(\hat \mu) = Y(A^T-D)v^\star(\hat \mu) = \nu^\star(\mu)(YA^T\ab^+ - 
			YD\ab^-) = \nu^\star(\hat \mu)b.
\]
...
For simplicity, we henceforth use $(v^\star,\nu^\star)$ and 
...
\begin{proof} 
	...
	Let $(v^\star,\nu^\star)$ be defined as in \eqref{opt-cond} and let $\gamma$
	be a positive fixed scalar. Define the set 
	\[
	\Omega = \left\{ (v,\nu)\in	\Re^{n}\times\R \; : \; (v,\nu)\geq 0, 
		\quad e^TYDv + e^TYA^T\ab^+\nu = \gamma\right\},
	\]  
	where $e$ is defined in \eqref{consis}.  By Brouwer's fixed-point
	theorem, if the parameter $(r,\rho) \in \Omega$ ensures that the corresponding 
    optimal solution $(v^\star,\nu^\star) \in \Omega$, then there is a fixed point 
    such that the parameter and solution are equal, i.e., there exists a $\mu$ 
    such that $\mu = (r,\rho) = (v^\star(\mu),\nu^\star(\mu))$.  

    The set $\Omega$ is bounded because all elements have non-negative entries and
    satisfy $(e^TYD, e^TYA^T\ab^+\nu)^T(v,\nu) = \gamma$, and from the assumptions
    on $Y$ and $K$ it follows that 
    $(e^TYD,e^TYA^T\ab^+)$ is a vector of positive entries. It is also formed
    by an intersection of closed convex sets, and hence is convex and compact.
    Moreover, the mapping $\mu\rightarrow (v^\star,\nu^\star)$ is continuous \cite{dantzig}.
    Since problem \eqref{convex-fix} is feasible for any $\mu\in\Omega$, the
    mapping $\Omega \ni \mu \rightarrow (v^\star,\nu^\star)$ is well defined.

	To show that the image of $\Omega$ under the mapping $(r,\rho) \rightarrow
	(v^\star,\nu^\star)$	is in $\Omega$, first observe that by the bounds in
    \eqref{convex-fix}, $(v^\star,\nu^\star) \ge 0$.  Using \eqref{massbalanceb}, 
    \eqref{opt-primal-feas} and Definition \ref{consis}
	we have
	\begin{align}
	e^T YD v^\star + e^T YA^T \ab^+ \nu^\star &= e^T YA^T r + e^T YD \ab^- \rho \notag 
	\\ & = e^T YD r + e^T YA^T \ab^+\rho=\gamma, \notag 
	\end{align} 
	and thus $(v^\star,\nu^\star)\in \Omega.$
	Therefore, under the mapping $(r,\rho) \rightarrow (v^\star,\nu^\star)$,
	$(r,\rho) \in \Omega$ implies $(v^\star,\nu^\star) \in \Omega$, and the
	mapping must have a fixed point.  Moreover, since $\Omega$ does not contain
	the zero vector, no fixed point is identically zero.
    \qed
\end{proof}

%Note that the value of $YDv^\star+YA^T\ab^+\nu^\star$ is the rate of
%consumption of each chemical species and $YA^Tv^\star+YD\ab^-\nu^\star$ is the
%rate of production of each chemical species. At the fixed point, the equality
%$YDv^\star + YA^T\ab^+\nu^\star= YA^Tv^\star+YD\ab^-\nu^\star$ defines a
%steady state. The set $\Omega$ defines the parameter $\gamma=e^T(YDv^\star +
%YA^T\ab^+\nu^\star)$, and since the vector $e$ can be interpreted as an assignment
...
By proving Theorem \ref{fp-exist-map} we have established the existence of a
nonzero fixed point $\mu$ of the mapping $\Omega \ni \mu \rightarrow
(v^\star,\nu^\star)\in \Omega$. Moreover, we have shown that when the
associated minimizer $(v^\star,\nu^\star)$ is positive, it is a solution to
\eqref{mak} and to $YKv^\star=\nu^\star b$.  However, in the case when some
...
\mu),\nu^\star(\hat\mu))$, and therefore $\hat\mu$, is positive.

Lemma $\ref{maximum-support}$ below shows that if problem $\eqref{convex-fix}$ has a
feasible point with support $J$, the minimizer $(v^\star,\nu^\star)$ will have
...
solution for which $\hat{\nu}=1$. This establishes that there is a nonzero
...
\begin{lemma} 
	...
	of the support of the minimizer $(v^\star,\nu^\star)$. 
	\label{maximum-support}
\end{lemma}

\begin{proof}
	Let $\tilde{v}\in \Re^{n+1}$ be any of the feasible points $(v,\nu)$ with the largest
	support and let $z$ be any feasible direction at $\tilde{v}$. By
	construction, for all $\alpha$ in some interval $[\alpha_1,\alpha_2]$ the points
	$v_\alpha:= \tilde{v} + \alpha z$ are non-negative and feasible.  The
	interval can be chosen so that when $\alpha=\alpha_1$ and when $\alpha=\alpha_2$, one new
	bound constraint becomes active.  This implies that $\supp(v_{\alpha_1})$ and
	$\supp(v_{\alpha_2})$ are strictly contained in $\supp(\tilde{v})$, and
	$\supp(v_\alpha) = \supp(\tilde{v})$ for $\alpha \in (\alpha_1,\alpha_2)$.
	
	Without loss of generality we assume $\alpha_1<0<\alpha_2$, since $\alpha_1$ and $\alpha_2$ will
	not be of the same sign, and if $\alpha_1 = 0$ and $\alpha_2>0$, any point $v_{\alpha}$ can
	be written as a convex combination of $\tilde{v}$ and $\tilde{v}+\alpha_2z$, and
	thus has support as large as $\tilde{v}$.

	Define the univariate function 
	\begin{equation}   
		\theta(\alpha) := \phi(\tilde{v} + \alpha z), 
		\label{univariate}
	\end{equation} 
	where $\phi$ is the objective function of \eqref{convex-fix}.  We will
	establish that as $\alpha\rightarrow \alpha_1$ the derivative $\theta'(\alpha)\rightarrow
	-\infty$, and as $\alpha\rightarrow \alpha_2$ the derivative $\theta'(\alpha)\rightarrow
	\infty$.  Thus, by the mean value theorem, there must exist a zero of the 
	function $\theta'$ in the interior of the interval $[\alpha_1,\alpha_2]$.
	...
	If we let $D = \diag(d_i)$ for $i = 1,\dots ,n$ and define
        $d_{n+1}=1$, we can write 
  \begin{align*}
	 \theta(\alpha) &=\sum_{i=1}^{n+1} (\tilde{v_i} + \alpha z_i) d_i\left(\log(\tilde{v}_i + \alpha z_i)-1\right).
  \end{align*} 
    ...
    of the entries that tend to zero as $\alpha \rightarrow \alpha_1$, then 
   \begin{align*} 
	 	\theta'(\alpha) &=\sum_{i\in J} z_i d_i(\log(\tilde{v}_i + \alpha z_i)) \\            
		&= \sum_{i\in (L^c\cap J)} z_id_i(\log{(\tilde{v_i}+\alpha z_i)}) + \sum_{i\in L}
		 z_id_i(\log{(\tilde{v_i}+\alpha z_i)}). 
	\end{align*} 
	As $\alpha \rightarrow \alpha_1$, the first summation will approach a finite value.
    ...
    entries that tend to zero as $\alpha\rightarrow \alpha_2$. Observe that for these
    entries, $z_i<0$ and
  \[
    \theta'(\alpha) = \sum_{i\in (U^c\cap J)} z_i d_i(\log{(\tilde{v_i}+\alpha z_i)})
  ...
  \]
	...
	$(\tilde{v},\tilde{\nu})$ with larger support than the minimizer
	$(v^\star,\nu^\star)$ of problem \eqref{convex-fix}.  Since
	$(v^\star,\nu^\star)$ has smaller support, we can write  $(v^\star,
	\nu^\star) = (\tilde{v},\tilde{\nu}) + \alpha^\star z$ where $\alpha^\star$
	...
	interior of the interval such that $(v^\star, \nu^\star) =
	(\tilde{v},\tilde{\nu}) + \hat \alpha z$ has a lower function value than
	$(v^\star,\nu^\star)$, contradicting its optimality.

	Therefore, by the mean value theorem, there must exist a stationary point of
	$\theta$ strictly in the interior of the interval $[\alpha_1,\alpha_2]$ at which the function
...
\begin{lemma}
	...
	exists a positive feasible point $(\hat v,\hat \nu)$.
\label{positive-feasible}
\end{lemma}

\begin{proof}
	Let problem \eqref{convex-fix} be parametrized with a fixed point $\hat\mu$,
	and let $(\hat v, \hat \nu)$ be both the minimizer and the fixed point.  We
	prove by contradiction that no entry of the minimizer $(\hat{v},\hat{\nu})$
	...
	First, assume that $\hat{\nu}>0$ and observe that $p := (D^{-1}A^T\hat v + \ab^-\hat \nu, 0)$ is 
	a feasible point. Since $\ab^-$ was chosen to be positive and 
	$D^{-1}A^T$ has no zero columns, the first $n$ entries 
	of the vector are positive. A convex combination of $(\hat v,\hat \nu)$ and $p$ will 
	be feasible and have full support.

	Now, assume that $\hat \nu = 0$ and some entry of $\hat v$ is nonzero, and
	...
	[0,1]$ the point  $(\tilde{v}, \tilde{\nu}) = \beta (D^{-1}A^T\hat{v},0) +
	...
	$((A^T)^p\hat{v}, 0)$ for all non-negative powers $p$.

    Powers of adjacency matrices have the property of representing paths of
    length equal to the power.  If $A$ is an adjacency matrix, row $i$ has nonzero
    entries in the indices of the nodes reachable in one step from $i$.
    Row $i$ of matrix $A^2$ has nonzero entries in the indices of the
    nodes reachable in exactly two steps from $i$. If the graph is weakly
    connected and formed by a single linkage class, then all nodes are
    reachable from all other by multiple step paths. Therefore, for any pair
    $(i,j)\in [1,\dots,n]\times [1,\dots,n]$, there exists a power $p$ large
    enough such that $(A^T)^p_{ij}>0$.  More importantly, if $\hat v_j > 0$,
    then there exists a $p$ such that $[(A^T)^p \hat v_j]_i > 0$ for all $i$.
    Therefore, if $\hat{\nu} = 0$ and $\hat{v} \neq 0$, there is a feasible
    point $(\tilde{v},0)$ such that $\tilde{v} > 0$.  
	
	Finally, if $\hat v > 0$ then there is a scalar $ 0<\alpha$ mall enough such
	that 
	\[
	0< \hat v - D^{-1}A^T\ab^+ \alpha,
	\]
	and then the equality  
	\[
		YD(\hat v - D^{-1}A^T\ab^+\alpha)+YA^T\ab^+\alpha = YD\hat v = YA^T\hat v
	\]
	implies that the positive point $(\hat v - D^{-1}A^T\ab^+\alpha,\alpha)$ is
	feasible.
...
\begin{proof}
	...
	that $Y^T \log(c) = \log (v)$ and $Y K v = \nu b$.  In other words, we have
...
Given an initial positive point $(\hat v, \hat \nu) \in \R^{n+1}$ and a small
tolerance $\tau$, we use the following fixed point iteration,
Algorithm~\ref{fp-iteration}, to find a parameter $\mu = (r, \rho)$ to
the problem \eqref{convex-fix} such that $(v^\star,\nu^\star) =
(r,\rho)$.
...
\label{fp-iteration}
\begin{algorithmic}[1]
  \STATE $(r,\rho) \gets (\hat v,\hat \nu)$
  \WHILE{$\|YK v^\star - b \nu^\star \|_\infty>\tau$}
  \STATE $(v^\star,\nu^\star)\gets \text{unique solution of \eqref{convex-fix}} $
		\label{min-step}
	\STATE $(r,\rho) \gets \frac{1}{2}(r,\rho) +\frac{1}{2}(v^\star,\nu^\star)$
  \ENDWHILE
  \label{fixpoint-alg}.
\end{algorithmic}
\end{algorithm}
...
converges to a fixed point $(r,\rho) = (v^\star,\nu^\star)$, then
...
$\gamma = e^T(YD v^\star + YA^T \ab^+ \nu^\star)$, as described in
...
across the boundary ($b=YK\ab$ for some $\ab \neq 0$).
...
the sequence of the infeasibilities $\norm{YKv_t-b \nu_t}_\infty$ 
