...
\begin{proof} 
    ...
    satisfy $(e^TYD, e^TYA^T\eta^+v_0)^T(v,v_0) = \gamma$, and from the assumptions
    ...
    Moreover, the mapping $\mu\rightarrow (v^\star,v^\star_0)$ is continuous \cite{dantzig}.
    Since problem \eqref{convex-fix} is feasible for any $\mu\in\Omega$, the
    mapping $\Omega \ni \mu \rightarrow (v^\star,v^\star_0)$ is well defined.
